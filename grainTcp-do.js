...
export default {
    fetch: async (req, env) => {
        ...
        const stub = await getDo(env, url.searchParams.get(`colo`) || req.cf.colo)
        if (stub) {
...
async function processReq(req, env) {
    ...
    if (req.headers.get('Upgrade')?.toLowerCase() === 'websocket') {
        ...
        return processNoneWebSocket(req);
    }
}
...
function getViewJSON(){
    return  JSON.stringify({
        "返袋": `${proxyCacheHost}`,
        "缓存返袋数组索引": `${proxyCacheIndex}`,
        "正在使用的返袋": `${proxyCacheList ? proxyCacheList[proxyCacheIndex].join(':'): ''}`,
        "缓存返袋解析数组": `${proxyCacheList ? proxyCacheList.map(item => item.join(':')) : ''}`
    }, null, 2);
}